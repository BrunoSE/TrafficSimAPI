...
//Se asume que link 1:2 existe y que no hay una zona en este link
//Se asume que ningun link fuente tiene nodo de cabeza semaforizado (Link con Zona no termina en interseccion con semaforo)
...
//Links sin zona tiene este numero como qpg_LNK_zone
#define numero_link_sin_zona 32768
...
void qpx_NET_postOpen(void) {
	...
	//Ver si variable numero_link_sin_zona esta bien definida
	if (qpg_LNK_zone(qpg_NET_link("1:2")) != numero_link_sin_zona)
	{
		qps_GUI_printf("%s\n", "ERROR: numero_link_sin_zona esta mal definido");
	}
	...
	{
		if (qpg_NDE_entryLinks(qpg_LNK_nodeEnd(qpg_NET_linkByIndex(i))) > 2 || qpg_NDE_exitLinks(qpg_LNK_nodeEnd(qpg_NET_linkByIndex(i))) > 2 || (qpg_LNK_match(qpg_NET_linkByIndex(i)) != qpg_NDE_link(qpg_LNK_nodeEnd(qpg_NET_linkByIndex(i)), 1) && qpg_LNK_match(qpg_NET_linkByIndex(i)) != qpg_NDE_link(qpg_LNK_nodeEnd(qpg_NET_linkByIndex(i)), 2)))
		{
...
void qpx_LNK_timeStep(LINK* link) {
	...
	if (EV_lider != NULL)
	{
		...
		if (qpg_LNK_zone(link) == numero_link_sin_zona && dist_min <= distancia_modelo_interseccion && map_link_tiene_interseccion[qpg_LNK_index(link)] == PTRUE)
		{
...
void qpx_VHC_transfer(VEHICLE* vehicle, LINK* link1, LINK* link2) {
	...
	if (qpg_VHC_type(vehicle) == n_tipo_EV)
	{
		...
		if (qpg_LNK_zone(link1) == numero_link_sin_zona && map_link_tiene_interseccion[qpg_LNK_index(link1)] == PTRUE)
		{
